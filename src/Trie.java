...
public class Trie {
	...
	private class TrieNode { // This is the class that represents a node in the trie
		...
		/**
		 * Counts all the words in the trie by recursively going into each subnode in the trie and seeing whether
		 * it is a valid end.
		 * 
		...
		public int countAllWords() {
			int numberOfWords = 0;
			// The following code looks through all the potential subnodes of the current node.
			for(int i = 0; i < 26; i++) {
				// If the subnode exists, the folliwng code is executed.
				if(subnodes[i] != null) {
					// If the word has a valid end, the number of words is incremented.
					if(subnodes[i].isValidEnd){
						numberOfWords++;
					}
					// The method is called recursively to add all the words inside of the subnode to the total.
					numberOfWords = numberOfWords + subnodes[i].countAllWords();
				}
			}
			// The number of words is returned up the call stack.
